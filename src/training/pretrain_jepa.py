...
def build_model_from_config(config: dict, num_joints: int, in_features: int) -> SkeletonJEPA:
    jc = config.get("jepa", {})
    return SkeletonJEPA(num_joints=num_joints, in_features=in_features, d_model=jc.get("d_model", 256), num_layers=jc.get("num_layers", 2), num_heads=jc.get("num_heads", 4), dropout=jc.get("dropout", 0.1), predictor_hidden_dim=jc.get("predictor_hidden_dim", 512), encoder_type=jc.get("encoder_type", "temporal_transformer"))
