...
class SkeletonTextDataset:
        ...
        for feat_idx in [0, 2, 4, 6]:
            if feat_idx == 0:
                ...
                if center_xy is not None:
                    for idx in indices:
                        if idx < 8:
                            pts = augmented[:, [idx], feat_idx:feat_idx+2] - center_xy[:, None, :]
                            augmented[:, [idx], feat_idx:feat_idx+2] = self._rotate_points_2d(pts, angle_rad) + center_xy[:, None, :]
                        else:
                            pts = augmented[:, [idx], feat_idx:feat_idx+2]
                            augmented[:, [idx], feat_idx:feat_idx+2] = self._rotate_points_2d(pts, angle_rad)
                else:
                    pts = augmented[:, indices, feat_idx:feat_idx+2]
                    augmented[:, indices, feat_idx:feat_idx+2] = self._rotate_points_2d(pts, angle_rad)
            else:
    ...
    def _augment_keypoints(self, keypoints: np.ndarray) -> np.ndarray:
        ...
        # Clip coordinates to a safe range to prevent float16 overflow in the LLM
        augmented[..., :8] = np.clip(augmented[..., :8], -3.5, 3.5)
